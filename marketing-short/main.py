...
def check_folder(path):
    # check folder tmp/ and subfolder tmp/images
...
def generate_video():
    ...
    for file in os.listdir("tmp/images"):
        if file.startswith("scene_") and file.endswith(".png"):
            image_path = f"tmp/images/{file}"
            order = file.split('.')[0].split('_')[1]
            video_prompt_path = f"tmp/images/scene_prompt_{order}.txt"
...
def develope_story(characters, setting, plot, number_of_scenes, duration_per_scene, style):
    ...
    with open("tmp/images/story.json", "w") as f:
    ...
    for i, scene in enumerate(json_response, 1):
        ...
        image.save(f"tmp/images/scene_{i}.png")
        video_prompt_file = f"tmp/images/scene_prompt_{i}.txt"
        with open(video_prompt_file, "w") as f:
            f.write(scene["description"]) # f.write(scene["video_prompt"])
        video_script_file = f"tmp/images/scene_script_{i}.txt"
...
def show_images_n_prompts():
    ...
    for i in range(6):
        if i < len(scene_image_files):
            generated_scene_images.append("tmp/images/" + scene_image_files[i])
        else:
    ...
    for i in range(6):
        if i < len(scene_prompt_files):
            generated_scene_prompts.append(open(f"tmp/images/{scene_prompt_files[i]}", "r").read())
        else:
...
def generate_audio():
    ...
    for file in os.listdir("tmp/images"):
        if file.startswith("scene_script_") and file.endswith(".txt"):
            ...
            string_script = open(f"tmp/images/{file}", "r").read()
...
with gr.Blocks(theme=gr.themes.Glass(), title="Story GeN/Video ") as demo:
    ...
    with gr.Tab("3. Visual Storyboarding >>"):
        ...
        with gr.Row():
            dd_generated_audios = gr.Dropdown(label="Generated audios", interactive=True)
        with gr.Row():
    ...
    demo.load(show_generated_audios, inputs=None, outputs=[dd_generated_audios])
    ...
    btn_generate_audios.click(generate_audio, inputs=None, outputs=[dd_generated_audios])
