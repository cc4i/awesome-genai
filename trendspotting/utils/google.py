...
# Generate keywords
def gen_keywords4google(context, model_id)->str:

    system_instruction = """
    You are a searching engine expert and specialized on searching for retail, analyze the provided context thoroughly and finish following tasks.
    """
    prompt = f"""
    *CONTEXT*
    {context}
    ...
    *TASKS*
    1. Identify the real, commercially products or catagories, do not include generic words, related accessories, or components for products.
    2. Generate the best Google search keywords, which could leads the best searching results.
    ...
    *OUTPUT FORMAT*
    ...
    """
    return call_llm(system_instruction, prompt, "", model_id)
...
def remove_noise_words(product:str, content:str):
    prompt = f"""
